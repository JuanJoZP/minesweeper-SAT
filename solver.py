...
class Solver:
    ...
    def __init__(self, size):
        # encoding:
        #   9 -> covered box
        #   0 to 8: box with respective number,
        #  -1: bomb (flag)
        self.board_matrix = []
        self.bomb_matrix = []
        self.bomb_in_box = Descriptor([size, size])
        self.rule = ""
    ...
    def solve(self) -> list[dict[str, bool]]:
        """checks the complete truth table and returns a list with all the satisfiable interpretations.
        An interpretation is of the form {"atom1": True, "atom2": False, ...}"""
        formula = inorder_to_tree(self.rule)
        if formula is None:
            raise ValueError("Empty rule")
        literals = formula.letras()
        if literals is None:
            raise ValueError("Formula with no literals")
    ...
    def initial_bomb_matrix(self):
        ...
        bombs: list[list[None | str]] = [
            [None] * len(self.board_matrix) for i in range(len(self.board_matrix[0]))
        ]
        for i in range(len(self.board_matrix)):
            for j in range(len(self.board_matrix[i])):
                ...
                if box in range(
                        1, 9
                ):  # if box is uncovered, creates atoms for all its adjacent cells
                ...
                for adjacent in adjacent_boxes:
                    ...
                    if (adjacent not in distribution):
                        possible_combination.append(
                            "-" + str(self.bomb_matrix[adjacent[0]][adjacent[1]]))
    ...
    def adjacent(self, x, y) -> list[tuple[int, int]]:
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                if x + i >= 0 and y + j >= 0 and x + i < len(
                        self.board_matrix) and y + j < len(self.board_matrix):
